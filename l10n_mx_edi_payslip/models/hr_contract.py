...
class HrContract(models.Model):
    ...
    def compute_integrated_salary_variable(self):
        ...
        for record in self:
            ...
            worked = sum(payslips.filtered(lambda p: p.struct_id.type_id.l10n_mx_edi_type == 'O').mapped(
                'worked_days_line_ids').filtered(lambda work: work.code == 'WORK100').mapped('number_of_days'))
            inputs = sum(payslips.mapped('line_ids').filtered('salary_rule_id.l10n_mx_edi_sdi_variable').mapped(
                'total'))
