...
class HRPayroll(PayrollTransactionCase):
    ...
    def test_001_xml_structure(self):
        ...
        self.env.ref('l10n_mx_edi_payslip.infonavit_qdp').active = False
        ...
        node_expected.Receptor.attrib[u'Antig\xfcedad'] = node_payroll.Receptor.attrib[u'Antig\xfcedad']
        ...
        self.assertEqualXML(self.xml_expected, xml)
    ...
    def check_inability_node(self, payroll, i_type, days):
        ...
        xml = payroll.l10n_mx_edi_get_xml_etree()
        self.assertEqual(
            i_type, payroll.l10n_mx_edi_get_payroll_etree(xml).Incapacidades.Incapacidad.get('TipoIncapacidad'),
            'Inability not added.')
        self.assertEqual(
            days, payroll.l10n_mx_edi_get_payroll_etree(xml).Incapacidades.Incapacidad.get('DiasIncapacidad'),
    ...
    def test_022_alimony(self):
        """Test case with alimony in the employee. Seven alimony types. Alimony expected amounts
           1. 110.0    2. 92.58    3. 100.0    4. 78.56
           5. 112.0    6. 85.63    7. 82.59
        """
        ...
        self.assertEqual(total_perceptions, 78400.0, 'Total Perceptions must be 78400.0')
        self.assertEqual(total_isr, 11469.99, 'Total Isr must be 11469.99')
        self.assertEqual(total_alimony, 661.36, 'Total Alimony must be 661.36')
    ...
    def test_023_mexican_holiday_allocation(self):
        ...
        self.assertTrue(
            self.env.ref('l10n_mx_edi_payslip.hr_rule_l10n_mx_payroll_perception_001_holidays'
                         ) in payroll.line_ids.mapped('salary_rule_id'), 'Holidays not paid.')
    ...
    def test_026_infonavit_loan_test(self):
        """Test the infonavit loans charges, fixed amount"""
        # TODO Cover more types
        self.contract.date_start = date(2020, 4, 1)
        self.contract.date_end = date(2020, 4, 30)
        loan = self.env.ref('l10n_mx_edi_payslip.infonavit_qdp')
        loan.date_from = date(2020, 4, 1)
        self.contract.state = 'open'
        payroll = self.create_payroll(date(2020, 4, 1), date(2020, 4, 15))
        payroll.compute_sheet()
        infonavit_rule = self.env.ref('l10n_mx_edi_payslip.hr_rule_l10n_mx_payroll_deduction_010')
        infonavit_loan_line = self.serach_rule_in_payroll(payroll, infonavit_rule, True)
        self.assertEqual(len(infonavit_loan_line), 1, 'There must be one infonavit loan line in the payslip')
        self.assertEqual(infonavit_loan_line.amount, 983.61, 'The infonavit amount is not what expected')

    def test_027_extra_hours(self):
        ...
        extra_taxed_line = self.serach_rule_in_payroll(payroll, extra_taxed_rule, True)
        extra_exempt_line = self.serach_rule_in_payroll(payroll, extra_exempt_rule, True)
        self.assertEqual(extra_taxed_line.total, extra_exempt_line.total,
                         'The extra hours amounts should be 50%% exempt 50%% taxed')
        # The quanity exempt is higher than 5 umas
        overtimes += self.env['hr.payslip.overtime'].create({
            'employee_id': self.employee.id,
        ...
        extra_taxed_line = self.serach_rule_in_payroll(payroll, extra_taxed_rule, True)
        extra_exempt_line = self.serach_rule_in_payroll(payroll, extra_exempt_rule, True)
        self.assertTrue(extra_taxed_line.total != extra_exempt_line.total,
                        'The extra hours amounts should not be 50%% exempt 50%% taxed')
        umas = self.contract.company_id.l10n_mx_edi_uma * 5
        self.assertEqual(extra_exempt_line.total, umas, 'The amount in the exempt should be 5 umas')
        taxed_expected = (self.contract.wage / 30.4 / 8 * 2 * sum(overtimes.mapped('hours'))) - umas
        self.assertTrue(float_compare(extra_taxed_line.total, taxed_expected, precision_digits=4),
                        'The amount in the exempt should be 5 umas')
        # The salary is the minimal
        self.contract.wage = self.contract.company_id.l10n_mx_edi_minimum_wage * 30.4
        payroll.compute_sheet()
        extra_taxed_line = self.serach_rule_in_payroll(payroll, extra_taxed_rule)
        self.assertFalse(extra_taxed_line, 'Should not be any extra hour taxed line')
        extra_exempt_line = self.serach_rule_in_payroll(payroll, extra_exempt_rule, True)
        amount_expected = self.contract.wage / 30.4 / 8 * 2 * sum(overtimes.mapped('hours'))
        self.assertTrue(float_compare(extra_exempt_line.total, amount_expected, precision_digits=4),
                        'The extra hours amount expected is %d' % amount_expected)
